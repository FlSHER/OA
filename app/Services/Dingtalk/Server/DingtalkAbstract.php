...
use App\Services\Dingtalk\DingtalkHelper;

// 实现公共方法
abstract class DingtalkAbstract
{
	use DingtalkHelper;
    ...
    protected function requestDingTalk($method, $uri, array $options = [])
    {
        ...
        $handler->push(function (callable $handler) {
            return function (RequestInterface $request, array $options) use ($handler) {
                return $handler($this->concat($request, ['access_token' => app('Dingtalk')->getAccessToken()]), $options);
            };
        });
    ...
    protected function requestTaobao($method, array $options = [])
    {
        if (! $handler = $this->taobaoHandlerStack) {
            ...
            $handler->push(function (callable $handler) {
                return function (RequestInterface $request, array $options) use ($handler) {
                    $query = [
                        'session' => app('Dingtalk')->getAccessToken(),
