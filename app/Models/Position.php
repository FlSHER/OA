...
class Position extends Model {
    ...
    public function department() { //所属部门  ps：职位调整为与品牌关联 by Fisher 2017-03-03
        return $this->belongsToMany('App\Models\Department', 'department_has_positions')
            ->orderBy('parent_id', 'asc')
            ->orderBy('sort', 'asc');
    }

    public function brand() { // 所属品牌
        return $this->belongsToMany('App\Models\Brand', 'brand_has_positions')
            ->orderBy('sort', 'asc')
            ->withTrashed();
    }

    public function authority() { //权限
        return $this->belongsToMany('App\Models\Authority', 'position_has_authorities')
            ->orderBy('parent_id', 'asc')
            ->orderBy('sort', 'asc');
    }
