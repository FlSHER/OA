...
class StaffRequest extends FormRequest {
    ...
    private function makeFormValidator($operationType) {
        ...
        if ($this->operationMatch($operationType, ['edit', 'entry', 'reinstate'])) {
            $formValidatorPersonal = [
                ...
                'relative.*.relative_sn' => ['required_with:relative.*.pivot.relative_name'],
                'relative.*.pivot.relative_type' => ['required'],
                'relative.*.pivot.relative_name' => ['required'],
            ];
        }
        if ($this->operationMatch($operationType, ['edit'])) {
            $formValidatorPersonal['relative.*.relative_sn'][] = 'different:staff_sn';
        }
    ...
    private function addPersonalValidator() {
        ...
        $validatorPersonal = [
            ...
            'mobile' => ['required', 'regex:/^1[34578][0-9]{9}$/', 'unique:staff,mobile,' . $staffSn . ',staff_sn,deleted_at,NULL', 'unique:staff,username,' . $staffSn . ',staff_sn,deleted_at,NULL'],
            ...
            'dingding' => ['max:20', 'unique:staff,NULL,' . $staffSn . ',staff_sn,deleted_at,NULL'],
