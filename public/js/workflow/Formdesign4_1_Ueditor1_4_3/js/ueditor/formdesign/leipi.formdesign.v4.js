...
UE.plugins['macros'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
        var leipiPlugins = el.getAttribute('leipiplugins');
        if ( (/input/ig.test( el.tagName ) || /select/ig.test( el.tagName ))&& leipiPlugins==thePlugins) {
            var html = popup.formatHtml(
 ...
 * start
 */
UE.plugins['mytime'] = function () {
    var me = this,thePlugins = 'mytime';
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/mytime.html',
                name:thePlugins,
                editor:this,
                title: '时间选择',
                cssRules:"width:600px;height:310px;",
