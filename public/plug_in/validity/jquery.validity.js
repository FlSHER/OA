...
///////////////////////////////////////////////////////////////////////////////
    ...
    $.validity = {
        ...
        messages: {
            ...
            // Format validators:
            match: "#{field} 格式不匹配",
            integer: "#{field} 必须是非负整数",
            date: "#{field} 必须是日期格式 (年/月/日)",
            email: "#{field} 必须是邮件地址格式",
            cny: "#{field} 必须是人民币格式",
            url: "#{field} 必须是网址格式",
            number: "#{field} 必须是数字",
            ...
            time24: "#{field} 必须是24小时制的时间 (如: 23:00)",
            time12: "#{field} 必须是12小时制的时间 (如: 12:00 AM/PM)",
            // Value range messages:
            lessThan: "#{field} 必须小于 #{max}",
            lessThanOrEqualTo: "#{field} 必须小于或等于 #{max}",
            greaterThan: "#{field} 必须大于 #{min}",
            greaterThanOrEqualTo: "#{field} 必须大于或等于 #{min}",
            range: "#{field} 必须在 #{min} 到 #{max} 之间",
            // Value length messages:
            tooLong: "#{field} 长度不能超过 #{max}",
            tooShort: "#{field} 长度不能少于 #{min}",
            // Composition validators:
            nonHtml: "#{field} 不能包括HTML标签",
            alphabet: "#{field} 包含禁止字段",
            minCharClass: "#{field} 中 #{charClass} 不能出现超过 #{max} 次",
            maxCharClass: "#{field} 中 #{charClass} 不能出现少于 #{min} 次",
...
///////////////////////////////////////////////////////////////////////////////
    ...
    $.fn.extend({
        ...
        validity: function (arg,callBack) {
            return this.each(
                    function () {
                            ...
                            if (arg) {
                                $(this).bind(
                                        ...
                                        function () {
                                            $.validity.start();
                                            f.apply(self);
                                            var end = $.validity.end().valid;
                                            if(end && $.isFunction(callBack)){
        ...
        match: function (rule, msg) {
            ...
            return validate(
                    ...
                    $.isFunction(rule) ?
                    function (obj) {
                        return !obj.value.length || rule(obj.value,obj);
                    } :
                    function (obj) {
...
//                                msg
//                                );
                        raiseError(this,msg); //edit by Fisher 2016-12-13
                    }
    ...
    $.validity.outputs.tooltip = {
        ...
        raise: function ($obj, msg) {
            ...
            var tooltip = $(
                    ...
                    "<div class=\"validity-tooltip-arrow\"></div>" +
                    "<div class=\"validity-tooltip-inner\">" + msg + "</div>" +
                    "</div>"
                    )
                    .click(function () {
                        $obj.focus();
